    ...
    if st.session_state.get('tmp_cal'):
        with st.form("form_cal_v26"):
            ...
            if st.form_submit_button("🔥 ANALYSER ET GÉNÉRER PRONOS"):
                ...
                # Calcul immédiat du prono pour éviter la KeyError
...
with tabs[2]:
    ...
    if jk_target and st.session_state['history'][s_active].get(jk_target):
        ...
        for p in data.get("pro", []):
            st.info(f"⚽ {p['m']} | Cotes: {p['c']}")
    else:
        st.write("Aucun pronostic disponible. Scannez un calendrier d'abord.")

# --- TAB 4 : RÉSULTATS (PROTECTION BORDURES) ---
with tabs[3]:
    ...
    if f_res:
        ...
        # Ajout de padding blanc pour le scan
        img_padded = ImageOps.expand(img_temp, border=50, fill='white')
        ...
        # Ajustement des coordonnées à cause du padding de 50px
        ...
        if extracted:
            with st.form("form_res_v26"):
                ...
                for i, r in enumerate(extracted):
                    ...
                    val_r.append({"h":h_team, "a":a_team, "s":c1.text_input("Score", r['s'], key=f"rs{i}"), "mt":c2.text_input("MT", r['mt'], key=f"rm{i}"), "hm":st.text_input("Buts D", r['hm'], key=f"rbh{i}"), "am":st.text_input("Buts E", r['am'], key=f"rba{i}")})
                if st.form_submit_button("✅ ENREGISTRER"):
                    ...
                    save_db(st.session_state['history']); custom_notify("Enregistré !")

# --- TAB 5 : HISTORIQUE ---
with tabs[4]:
    for jk in sorted(st.session_state['history'][s_active].keys()):
        with st.expander(f"📅 {jk}"):
            d = st.session_state['history'][s_active][jk]
            if d.get("res"): st.table(pd.DataFrame(d["res"]))
