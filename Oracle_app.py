...
st.set_page_config(page_title="Oracle V17.7 - Ticket Expert", layout="wide")
...
st.title(f"🔮 ORACLE V17.7")

tabs = st.tabs(["🌟 SAISON", "📅 CALENDRIER", "🎯 PRONOS & TICKETS", "⚽ RÉSULTATS"])

# --- TAB 0 & 1 : (IDENTIQUES V17.6 - AUCUN CHANGEMENT) ---
with tabs[0]:
    ...
    if 'cal_v16' in st.session_state:
        with st.form("f_cal"):
            ...
            for i, m in enumerate(st.session_state['cal_v16']):
                ...
                v1 = c1.number_input("C1", value=m['o'][0], key=f"c1_{i}")
                vx = cx.number_input("CX", value=m['o'][1], key=f"cx_{i}")
                v2 = c2.number_input("C2", value=m['o'][2], key=f"c2_{i}")
                validated.append({'h': hf, 'a': af, 'o': [v1, vx, v2]})
            if st.form_submit_button("🔥 VALIDER ET ANALYSER"):
                st.session_state['ready_v16'] = validated

# --- TAB 2 : PRONOS & TICKETS (MISE À JOUR DEMANDÉE) ---
with tabs[2]:
    if 'ready_v16' in st.session_state:
        st.subheader("📊 Prédictions Scores Probables")
        
        # Listes pour construire les tickets
        ...
        for m in st.session_state['ready_v16']:
            ...
            # Affichage de l'analyse individuelle
            st.write(f"⚽ **{m['h']} {score_txt} {m['a']}**")

            # Sélection pour Ticket SAFE (Cotes < 1.7)
            if m['o'][0] < 1.7:
                safe_data.append({"txt": f"{m['h']} {score_txt} {m['a']}", "cote": m['o'][0]})
            elif m['o'][2] < 1.7:
                safe_data.append({"txt": f"{m['h']} {score_txt} {m['a']}", "cote": m['o'][2]})
            
            # Sélection pour Ticket RISQUE (Cotes entre 1.7 et 3.0)
            if 1.7 <= m['o'][0] <= 3.0:
                risque_data.append({"txt": f"{m['h']} {score_txt} {m['a']}", "cote": m['o'][0]})
            elif 1.7 <= m['o'][2] <= 3.0:
                risque_data.append({"txt": f"{m['h']} {score_txt} {m['a']}", "cote": m['o'][2]})
            
            # Sélection pour Ticket FUN (Nuls ou grosses cotes > 3.0)
            if m['o'][1] > 3.5:
                fun_data.append({"txt": f"{m['h']} {score_txt} {m['a']} (Nul)", "cote": m['o'][1]})

        st.divider()
        st.subheader("🎟️ Vos Tickets du Jour")
        c1, c2, c3 = st.columns(3)

        # Fonction pour afficher un ticket
        def afficher_ticket(col, titre, data, style):
            with col:
                if style == "safe": st.success(f"🟢 **{titre}**")
                elif style == "risque": st.warning(f"🟡 **{titre}**")
                else: st.error(f"🔴 **{titre}**")
                ...
                for i, match in enumerate(data[:3]): # Limité à 3 matchs par ticket pour la clarté
                    st.write(f"*{match['txt']} : Cote **{match['cote']}**")
                    total_cote *= match['cote']
                    calcul_str += f"{match['cote']}" + (" x " if i < len(data[:3])-1 else "")
                
                st.write("---")
                st.write(f"**Total de cote = {calcul_str} = {round(total_cote, 2)}**")

        afficher_ticket(c1, "TICKET SAFE", safe_data, "safe")
        afficher_ticket(c2, "TICKET RISQUE", risque_data, "risque")
        afficher_ticket(c3, "TICKET FUN", fun_data, "fun")

# --- TAB 3 : RÉSULTATS (STRICT V17.2 - AUCUN CHANGEMENT) ---
with tabs[3]:
