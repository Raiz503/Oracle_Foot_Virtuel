...
st.set_page_config(page_title="Oracle V17.6 - Partial Scan Fix", layout="wide")

@st.cache_resource
def load_ocr():
    # Utilisation de detail=1 pour avoir les coordonnées si besoin à l'avenir
...
# --- TAB 1 : CALENDRIER (Base V12 avec Fix "Cotes Coupées") ---
with tabs[1]:
    file_cal = st.file_uploader("📸 Scan Calendrier", type=['jpg','png','jpeg'], key="up_cal")
    if file_cal:
        # Lecture sans détails pour la rapidité (style V12)
        ...
        for t in res:
            ...
            # Regex robuste pour capturer les cotes même en bordure
            nums = re.findall(r"\d+[\.,]\d+", t)
            if nums:
                for n in nums:
                    val = float(n.replace(',', '.'))
                    if 1.01 <= val <= 60.0: # Filtre large pour les grosses cotes
                        f_odds.append(val)
        ...
        for i in range(10):
            ...
            # --- FIX MATCH 10 / COTES PARTIELLES ---
            # On récupère la tranche de 3 cotes
            chunk = f_odds[i*3:i*3+3]
            
            # On remplit ce qu'on trouve, sinon on met 1.0 par défaut (facile à éditer)
            o1 = chunk[0] if len(chunk) > 0 else 1.0
            ox = chunk[1] if len(chunk) > 1 else 1.0
            o2 = chunk[2] if len(chunk) > 2 else 1.0
            
            st.session_state['cal_v16'].append({'h': h, 'a': a, 'o': [o1, ox, o2]})
    ...
    if 'cal_v16' in st.session_state:
        with st.form("f_cal"):
            ...
            for i, m in enumerate(st.session_state['cal_v16']):
                ...
                # Champs éditables pour corriger la cote manquante (C2 du dernier match)
                v1 = c1.number_input("C1", value=m['o'][0], key=f"c1_{i}", format="%.2f")
                vx = cx.number_input("CX", value=m['o'][1], key=f"cx_{i}", format="%.2f")
                v2 = c2.number_input("C2", value=m['o'][2], key=f"c2_{i}", format="%.2f")
...
with tabs[2]:
    if 'ready_v16' in st.session_state:
        ...
        for m in st.session_state['ready_v16']:
            ...
            if m['o'][0] < 1.6 or m['o'][2] < 1.6: t_safe.append(f"{m['h']} ou {m['a']}")
            if 1.7 <= m['o'][0] <= 2.6: t_risque.append(f"V. {m['h']}")
            if m['o'][1] > 3.6: t_fun.append(f"Nul {m['h']}-{m['a']}")

        st.divider()
        c1, c2, c3 = st.columns(3)
        with c1: st.success("🟢 **SAFE**\n\n" + "\n".join(t_safe[:4]))
        with c2: st.warning("🟡 **RISQUE**\n\n" + "\n".join(t_risque[:3]))
        with c3: st.error("🔴 **FUN**\n\n" + "\n".join(t_fun[:3]))

# --- TAB 3 : RÉSULTATS (STRICT V17.2 - NE PAS CHANGER) ---
with tabs[3]:
